    ...
    <form id="invoiceForm" action="{{ route('store_zatca') }}" method="POST" class="zatca-form">
        ...
        <div class="zatca-row">
            <div class="zatca-form-group">
                ...
                <select class="zatca-input zatca-select" id="buyer_id" name="buyer_id">
        ...
        <div class="zatca-row">
            <div class="zatca-form-group">
                <label for="invoice_date" class="zatca-label">From Date</label>
                <input type="date" class="zatca-input" id="invoice_from_date" name="invoice_from_date" required>
            </div>
            <div class="zatca-form-group">
                <label for="invoice_date" class="zatca-label">To Date</label>
                <input type="date" class="zatca-input" id="invoice_to_date" name="invoice_to_date" required>
            </div>
        </div>
        <!-- Invoice Items -->
    ...
    $(document).ready(function() {
        ...
        function calculateInvoiceSummary() {
            ...
            $('#invoice-items tr').each(function() {
                ...
                const total = ((quantity * price) - discount).toFixed(3);

                totalPriceBeforeTax += parseFloat(total);
                totalTax += parseFloat(((taxPercent / 100) * total).toFixed(3));
                totalPriceAfterTax += parseFloat(total) + parseFloat(((taxPercent / 100) * total)
                    .toFixed(3));
            });

            $('#total-price-before-tax').text(totalPriceBeforeTax.toFixed(3));
            $('#total_before_tax').val(totalPriceBeforeTax);
            $('#total-tax').text(totalTax.toFixed(3));
            $('#total_tax').val(totalTax);
            $('#total-price-after-tax').text(totalPriceAfterTax.toFixed(3));
        ...
        function calculateItemTax(row) {
            const id = row.find('[name="item_ids[]"]').val();
            const quantity = parseFloat(row.find(`[name="selected_products[${id}][quantity]"]`).val()) || 0;
            const price = parseFloat(row.find(`[name="selected_products[${id}][price]"]`).val()) || 0;
            const taxPercent = parseFloat(row.find(`[name="selected_products[${id}][tax_percent]"]`).val()) ||
                0;
            const discount = parseFloat(row.find(`[name="selected_products[${id}][discount]"]`).val()) || 0;

            const tax = ((price * taxPercent) / 100).toFixed(3);
            const total = ((price) + parseFloat(tax) - discount).toFixed(3);

            row.find(`[name="selected_products[${id}][tax]"]`).val(tax);
            row.find(`[name="selected_products[${id}][total]"]`).val(total);
        }
        ...
        $('#item_id').on('change', function() {
            ...
            const item = {
                ...
                price: parseFloat(selectedOption.data('price')).toFixed(3),
                tax: parseFloat(selectedOption.data('tax')).toFixed(3),
                taxPercent: parseFloat(selectedOption.data('tax-percent')).toFixed(3),
                total: parseFloat(selectedOption.data('total')).toFixed(3)
            };
            ...
            if (item.id && item.name && item.price !== undefined && item.tax !== undefined && item
                ...
                const row = `
                    <tr>
                        ...
                        <td><input type="number" step="0.001" name="selected_products[${item.id}][price]" class="form-control zatca-input" value="${item.price}"></td>
                        <td><input type="number" step="0.001" name="selected_products[${item.id}][discount]" class="form-control zatca-input" value="0"></td>
                        <td><input type="number" step="0.001" name="selected_products[${item.id}][tax]" class="form-control zatca-input" value="${item.tax}"></td>
                        <td><input type="number" step="0.001" name="selected_products[${item.id}][tax_percent]" class="form-control zatca-input" value="${item.taxPercent}" readonly></td>
                        <td><input type="number" step="0.001" name="selected_products[${item.id}][total]" class="form-control zatca-input" value="${item.total}" readonly></td>
                        <td><button type="button" class="btn btn-danger zatca-btn-remove">Remove</button></td>
                    </tr>
                    <tr>
                        <td colspan="8"><textarea name="selected_products[${item.id}][note]" class="form-control zatca-textarea" placeholder="Item note..."></textarea></td>
                    </tr>
                     `;
