...
class ImportSalesController extends Controller
{
    ...
    public function fix_sales()
    {
        ...
        foreach ($transactions3 as $transaction) {
            if ($transaction->payment_status == 'due') {
                $paymentInput['amount'] = $transaction->final_total;
                $paymentInput['method'] = 'cash';
                $paymentInput['paid_on'] = $transaction->transaction_date;
                $paymentInput['created_by'] = auth()->user()->id ?? null;
                $paymentInput['transaction_id'] = $transaction->id;
                $paymentInput['business_id'] = $transaction->business_id;
            ...
            if ($transaction->payment_status == 'partial') {
                $transactionPayments = TransactionPayment::where('transaction_id', $transaction->id)->get();
                $paid = 0;
                foreach ($transactionPayments as $transactionPayment) {
                    $paid += $transactionPayment->amount;
                }

                $paymentInput['amount'] = $transaction->final_total - $paid;
                $paymentInput['method'] = 'cash';
                $paymentInput['paid_on'] = $transaction->transaction_date;
                $paymentInput['created_by'] = auth()->user()->id ?? null;
                $paymentInput['transaction_id'] = $transaction->id;
    ...
    private function __formatSaleData($imported_data, $import_fields, $group_by)
    {
        ...
        $payment_paid_key = array_search('payment_paid', $import_fields);
        ...
        foreach ($imported_data as $key => $value) {
            ...
            $formatted_array[$key]['payment_paid'] = $payment_paid_key !== false ? $value[$payment_paid_key] : null;
            ...
            // error_log(json_encode($formatted_array[$key]));
    ...
    private function __importFields()
    {
        $fields = [
            ...
            'payment_paid' => ['label' => __('lang_v1.payment_paid')],
        ];
